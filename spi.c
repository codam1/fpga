...
#define SPI1_DELAY1 *(volatile uint32_t*)0x1002402C
...
//
//	printf("%s\n", " | spi_xfer_recv_header: sending: %02x %02x %02x %02x\r\n",
//			at_flag_buf[0], at_flag_buf[1], at_flag_buf[2], at_flag_buf[3]);
//
//	for (uint32_t i = 0; i < 4; i++) {
//		while (SPI1_TXDATA > 0xFF) {
//		} // full bit set, wait
...
//
//	cs_deassert();
//	printf("%s\n", " | Waiting for handshake pin ready...");
//	fflush(stdout);
...
//	len_buf[1] = len >> 7;
//
//	printf("%s\n", " | spi_send length: sending: (%u) %02x %02x %02x %02x\r\n",
//			(len_buf[1] << 7) + len_buf[0], len_buf[0], len_buf[1], len_buf[2],
//			len_buf[3]);
//
//	for (uint32_t i = 0; i < 4; i++) {
//		while (SPI1_TXDATA > 0xFF) {
...
void spi_read_fram(const char *str_p, char *str_rx, int len) {
	//uint32_t len;
...
//----------------------------------------------------------------------
// Send a string  on SPI to Fram
//----------------------------------------------------------------------
void spi_send_fram(const char *str_p, char *str_rx, int len) {
	//uint32_t len;
...
//		printf("%s\n", " | -- Transparent mode ENABLED. End with \"+++\"\r\n");
...
//	}
	...
	for (uint32_t i = 0; i < len; i++) {
		...
		str_rx[i] = spi_rxdata_read();
		//}
	}
	SPI1_CSMODE = CS_AUTO;
//	memcpy(str_rx,data_in,sizeof(uint8_t)*50);
...
//
//	printf("%s\n", "[+] spi_xfer_send_header: sending: %02x %02x %02x %02x\r\n",
//			at_flag_buf[0], at_flag_buf[1], at_flag_buf[2], at_flag_buf[3]);
//
//	for (uint32_t i = 0; i < 4; i++) {
//		while (SPI1_TXDATA > 0xFF) {
