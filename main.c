...
int BUTTON_INPUT(uint8_t ATTEMPTS_COUNT) {
	...
	char butState = 'A';
	uint8_t tempCount[5];
	read_bytes(0x28, NULL, &tempCount, 1);
	...
	while (exit == 0) {
		...
		case 'E':
			write_bytes(0x28, tempCount, NULL, 1);
			read_bytes(0x28, NULL, &check, 1);
...
int main() {
	...
	if (ATTEMPTS_COUNT[0] > ATTEMPTS_LIMIT) {
		printf("%s\n", "Too many attempts , LOCK enabled \n");
		metal_gpio_enable_output(gpio0, 1);
		metal_gpio_toggle_pin(gpio0, 1);
		exit(0);
	}
